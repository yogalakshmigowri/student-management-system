...
class Management:
    ...
    def Checkdate_View(self):
        if self.getInfo_entry.get() == "":
            ...
            try:
                connection = pymysql.connect(host=self.host, user=self.user, password=self.password, database=self.database)
                curs = connection.cursor()
                curs.execute("select * from student_register where c_date=%s", self.getInfo_entry.get())
                row=curs.fetchone()
    ...
    def GetUpdateDetails(self, row):
        ...
        self.contact_data = Label(self.frame_2, text=row[8], font=(self.font_1, 10)).place(x=100, y=290)
        #self.contact_entry = Entry(self.frame_2, bg=self.color_4, fg=self.color_3)
        #self.contact_entry.insert(0, row[8])
        #self.contact_entry.place(x=100,y=410, width=200,height=22)
        ...
        self.date_entry.get() == "" or self.age_entry.get() == "" or self.gender_entry.get() == "" or self.birth_entry.get() == "" or self.email_entry.get() == "" or
            ...
            try:
                ...
                if row==None:
                    ...
                    curs.execute("update student_register set f_name=%s,l_name=%s, course=%s, course_Package=%s, c_date=%s, age=%s, gender=%s, birth=%s,email=%s,current_course=%s,pending_course=%s,completed_course=%s,total_fees=%s,paid_fees=%s,balance_fees=%s where contact=%s",
                                        (
                                            self.name_entry.get(),
    ...
    def Submit(self):
        ...
        or self.email_entry.get() == "" or self.Current_Course_entry.get()== "" or self.Pending_Course_entry.get() == "" or  self.Completed_Course_entry.get()== ""
        or self.Total_Fees_entry.get()== "" or
            self.Paid_Fees_entry.get()== "" or self.Balance_Fees_entry.get()== ""):
            messagebox.showerror("Error!","Sorry!, All fields are required",parent=self.window)
        else:
            try:
                ...
                if row!=None:
                    ...
                    curs.execute("insert into student_register (f_name,l_name,course,course_package,c_date,age,gender,birth,contact,email,current_course,pending_course,completed_course,Total_fees,paid_fees,balance_fees)values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                        (
                                            self.name_entry.get(),
                                            self.surname_entry.get(),
                                            self.course_entry.get(),
